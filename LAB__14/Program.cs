...
namespace lab14
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            ThreadingCounter();
            //4
            Thread OddThread = new Thread(odd);//Помещаем созданный метод в делегат
            Thread EvenThread = new Thread(even);
            OddThread.Start();//Запускаем потоки
            EvenThread.Start();
            ...
            EvenThread1.Priority = ThreadPriority.Highest;//Расставляем приоритеты. Другими словами указываем программе, какому потоку ей стоит больше уделять внимание
